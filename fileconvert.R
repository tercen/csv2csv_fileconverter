...
# split files into pieces, if neccessary
if (size_of_input_file_in_megs <= size_of_file_limit_in_megs) { 
  ...
  for (i in 1:num_pieces) {
    ...
    if (i == num_pieces) end_index <- nrow(dataset)
    
    print (paste("piece", i, "rows from",  start_index, "to", end_index))
    
    piece_name <- paste0(file_part$path_without_ext, "_", i, ".", file_part$ext)
    print(piece_name)
    
    if (file.exists(piece_name)) file.remove(piece_name)
